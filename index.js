...
//
// Alexa Skill - Damaris Blas
//

// sets up dependencies
const Alexa = require('ask-sdk-core'); //se le asignálas variables ask
...
const GetNewFactHandler = {
  ...
  handle(handlerInput) {
    ...
    return handlerInput.responseBuilder
      //.speak(speakOutput)
      .speak("Bienvenido a preguntas de arquitectura de software,acá podras memorizar conceptos para el examen. Cuantas preguntas deseas memorizar? ")
      // Uncomment the next line if you want to keep the session open so you can
      // ask for another fact without first re-opening the skill
      // .reprompt(requestAttributes.t('HELP_REPROMPT'))
      //.withSimpleCard(requestAttributes.t('SKILL_NAME'), randomFact)
      .reprompt("Elige cuantas preguntas deseas memorizar")
...
const PreguntarHandler = {
  canHandle(handlerInput) {
    const request = handlerInput.requestEnvelope.request;
    return request.type === 'IntentRequest'
      && request.intent.name === 'Respuesta';
  },
  handle(handlerInput) {
    const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
    const respuestaSlot =  handlerInput.requestEnvelope.request.intent.slots.NumeroRespuestaSlot.value;
    var respuestal;
    let pregunta,respuesta;
    let randomFact = requestAttributes.t('RESONDER');
    
    if (respuestaSlot != undefined){
     let pre= randomFact.split('+');
      pregunta= pre[0];
      respuesta= pre[1];
    }
    return handlerInput.responseBuilder
      .speak(requestAttributes.t('Pregunta numero 1 del dia  '+pregunta+ ' la respuesta a esta pregunta es    '+respuesta))
